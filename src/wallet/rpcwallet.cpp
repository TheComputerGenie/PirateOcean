    ...
    */
    ...
    if ( (keylen= (int32_t)strlen(params[0].get_str().c_str())) > 0 )
    {
        ...
        if (!destaddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid dest Pirate address");
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            ...
            if ( !ExtractDestination(tmpspk, dest, true) )
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Pirate address or pubkey: ") + name_);
        }
        CScript scriptPubKey = GetScriptForDestination(dest);
...
UniValue encryptwallet(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    StartShutdown();
    return "wallet encrypted; Pirate server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
// mrlych got bored and copied str4d's work - thanks dude/dudette :)
